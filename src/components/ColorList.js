...
function GetPreviousPage(currentPage){
  if (currentPage != 0) {
    return <Link to={"/colors/" + (parseInt(currentPage) - 1) }><span style={{margin:'10px'}}>previous</span></Link>
  }
}

function GetNextPage(currentPage, listLength){
  if (listLength >= 5) {
    return <Link to={"/colors/" + (parseInt(currentPage) + 1) }><span style={{margin:'10px'}}>next</span></Link>
  }
}
